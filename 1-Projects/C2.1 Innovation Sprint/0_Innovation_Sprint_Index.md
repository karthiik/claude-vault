# Innovation Sprint

#project/innovation-sprint #area/career

> **Goal:** Establish Lockton's innovation rhythm — turning hackathon energy into sustained, visible progress.

---

## Status

| | |
|---|---|
| **Owner** | Karthik Ramadoss |
| **Sponsor** | Byron |
| **Started** | January 2026 |
| **Target** | Q1 2026 |

---

## Goal Alignment

| Code | Goal | How This Project Contributes |
|------|------|------------------------------|
| **[[Goals-Reference#C2\|C2]]** | Innovation Leadership | Establishes repeatable innovation rhythm — sprint framework becomes proof of innovation value |
| **[[Goals-Reference#C1\|C1]]** | External Visibility | Success story for speaking/articles — "How we built an innovation culture at Lockton" |

---

## Context

Coming out of the 2026 GAS Summit and Hackathon, Byron asked for:
1. **Synthesis** of Summit & Hackathon survey results → executive deck
2. **Innovation Sprint plan** — a repeatable framework for driving innovation

---

## Key Deliverables

- [ ] Synthesize Summit & Hackathon survey results → deck for Byron #p1
- [ ] Create Innovation Sprint plan/framework → deliver to Byron #p1
- [ ] Define sprint cadence and governance model
- [ ] Identify pilot teams and initial sprint scope

---

## Open Questions

- What's the ideal sprint duration? (2 weeks? 4 weeks?)
- How do we balance innovation work with BAU delivery?
- Who are the innovation champions in each region?
- How do we measure innovation outcomes?

---

## Related

- [[1-Projects/2026 GAS Summit/2026 GAS Summit Reflection|2026 GAS Summit Reflection]]
- [[HIVE]] — potential pilot for innovation sprint model

---

## Log

### 2026-01-21
- Project created
- Tomorrow's #1 priority: Rally the troops around the sprint framework

